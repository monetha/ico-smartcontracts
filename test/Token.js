...
contract('token', accounts => {
  ...
  it("should transfer 20MTH from the owner to acc1", async() => {
    let result = await instance.transferFrom(accounts[0], accounts[1], 2000000, {from: accounts[1]});
    let event = result.logs[0].args;
  ...
  it("should fail to transfer from acc1, because trading not yet enabled", async() => {
    try {
      let result = await instance.transfer(accounts[2], 5000, {from: accounts[1]});
      throw new Error('Promise was unexpectedly fulfilled. Result: ' + result);
    } catch (error) {
      try {
        let result = await instance.transferFrom(accounts[1], accounts[2], 5000, {from: accounts[1]});
        throw new Error('Promise was unexpectedly fulfilled. Result: ' + result);
      } catch (err) {
      }
    }
  ...
  it("should transfer from acc1 to acc2", async() => {
    ...
    let result = await instance.transfer(accounts[2], 1000000, {from: accounts[1]});
  ...
  it("should fail to transfer from acc1 because of insufficient funds", async() => {
    try {
      let result = await instance.transfer(accounts[2], 6000000, {from: accounts[1]});
      throw new Error('Promise was unexpectedly fulfilled. Result: ' + result);
    } catch (error) {
    }
  });
  ...
  it("should fail to spend the locked tokens", async() => {
    try {
      let result = await instance.transfer(accounts[7], 5000000000000, {from: owner});
      throw new Error('Promise was unexpectedly fulfilled. Result: ' + result);
    } catch (error) {
      let bal = await instance.balanceOf.call(accounts[7]);
      assert(bal, 5000000000);
